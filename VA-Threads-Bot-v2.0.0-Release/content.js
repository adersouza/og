        ...
        if (!attachBtn) {
          ...
          if (validFiles.length > 0) {
            ...
            if (intercepted) {
              ...
              console.log(`📸 Found ${mediaPreviews.length} media previews in composer`);
  ...
  for (const spoiler of spoilerPositions) {
    ...
    if (startNode && endNode) {
      try {
        ...
        // ✅ Use selectors from SELECTORS (auto-updateable)
        const spoilerSelectors = SELECTORS['spoiler.hideButton'] || ['[aria-label*="spoiler" i]'];
        let spoilerButton = null;
        for (const selector of spoilerSelectors) {
          spoilerButton = document.querySelector(selector);
          if (spoilerButton) break;
        }
        
        if (spoilerButton) {
          console.log("Found spoiler button in toolbar");
          spoilerButton.click();
          await new Promise(resolve => setTimeout(resolve, 300));
        } else {
          // ✅ MÉTHODE 2 : Essayer le clic droit
          console.log("Trying right-click method...");
          
          // Obtenir les coordonnées du milieu de la sélection
          ...
          if (rects.length > 0) {
            ...
            // Envoyer l'événement au textArea
            textArea.dispatchEvent(contextMenuEvent);
            console.log("Right-click sent at:", x, y);
            
            // Attendre le menu
            await new Promise(resolve => setTimeout(resolve, 500));
            
            // Chercher l'option spoiler dans le menu contextuel
            const menuOptions = Array.from(document.querySelectorAll('*')).filter(el => {
              const text = el.textContent?.toLowerCase();
              return text && (
                text.includes('spoiler') ||
                text.includes('hide text') ||
                text.includes('hidden')
              ) && el.offsetParent !== null; // Vérifier que c'est visible
            });
            
            if (menuOptions.length > 0) {
              console.log(`Found ${menuOptions.length} spoiler options`);
              menuOptions[0].click();
              await new Promise(resolve => setTimeout(resolve, jitter(300, 0.2)));
            } else {
