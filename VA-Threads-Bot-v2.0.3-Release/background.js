// Import firebase-config, License Manager and Client Executors
importScripts('firebase-config.js', 'license-manager.js', 'client-executor.js');
...
class StorageService {
  async getSettings() {
    ...
    // ✅ FIX: Merge with defaults to ensure all fields exist (for new features)
    // This ensures export always includes ALL settings, even newly added ones
    const defaults = await this.loadDefaults();
    const mergedSettings = { ...defaults, ...result };
    ...
    if (localData.postsRaw !== undefined) {
      mergedSettings.postsRaw = localData.postsRaw;
    } else if (!mergedSettings.postsRaw) {
      mergedSettings.postsRaw = ''; // Default value
    }
 ...
 * ✅ FIXED: Correct timezone calculation using Intl.DateTimeFormat
 */
function getTodayMidnightTimestamp(timezone) {
  const now = Date.now();
  ...
  // Calculate milliseconds since midnight in target timezone
  const msSinceMidnight = (hour * 3600 + minute * 60 + second) * 1000;

  // Subtract to get exact timestamp of today's midnight in target timezone
  const midnight = now - msSinceMidnight;

  // Round down to nearest second to avoid millisecond drift
  return Math.floor(midnight / 1000) * 1000;
}

/**
 * Get today's date string (YYYY-MM-DD) in user's timezone
 * ✅ FIXED: Uses correct timezone calculation
 */
 ...
 * ✅ FIXED: Correct timezone calculation
 */
...
class MediaManager {
  ...
  parseMediaTag(tag) {
    ...
    // [[media:1-10]] → range
    // [[media:pair:random]] → random pair of consecutive photos

    const content = tag.replace(/\[\[media:|]]/g, '');

    // Pair random
    if (content === 'pair:random') {
      return {
        type: 'pair:random'
      };
    }
  ...
  async resolveMediaTag(tag, profileId = 'default') {
    ...
    switch (parsed.type) {
      case 'pair:random':
        // Random pair selection (consecutive photos: 1+2, 3+4, 5+6, etc.)
        // Sort media by numeric name
        ...
        const numberOfPairs = Math.floor(sortedMedia.length / 2);

        if (numberOfPairs > 0) {
          // Pick a random pair (0 to numberOfPairs-1)
          const randomPairIndex = Math.floor(Math.random() * numberOfPairs);

          // Get the two photos for this pair (consecutive indices)
          const photo1Index = randomPairIndex * 2;     // 0, 2, 4, 6, 8, ...
          const photo2Index = randomPairIndex * 2 + 1; // 1, 3, 5, 7, 9, ...

          selectedMedia = [sortedMedia[photo1Index], sortedMedia[photo2Index]];

          console.log(`[MEDIA] Pair random: Selected pair ${randomPairIndex + 1}/${numberOfPairs} (photos: ${sortedMedia[photo1Index].name}, ${sortedMedia[photo2Index].name})`);
        } else {
          console.warn('[MEDIA] Pair random: Not enough photos for a complete pair');
        }
        break;
...
class BackgroundApp {
  ...
  async handleStart() {
    ...
    // ✅ IMPROVEMENT: Ensure DAILY_RESET alarm is scheduled
    // If extension was reloaded or alarm was lost, reschedule it
    const existingAlarm = await chrome.alarms.get(ALARM.DAILY_RESET);
    if (!existingAlarm) {
      ...
      await this.log.info('DAILY_RESET_SCHEDULED', `Daily reset alarm was missing, rescheduled for ${new Date(tomorrowTimestamp).toISOString()} (${userTimezone})`, { module: 'APP' });
    }
  ...
  async handleExportSettings() {
    ...
    try {
      ...
      // ✅ FIX: Properly wait for transaction completion
      ...
      console.error('❌ Failed to export media data:', error);
      // ✅ Still return success but with null mediaData (settings will export)
    }
  ...
  async handleHeartbeat() {
    try {
      ...
      // ✅ Check if it's a license error (not network error)
      if (error.isLicenseError) {
        console.log('💓 Heartbeat: License error detected, stopping bot -', error.message);
  ...
  async resolveMediaTags(postText, profileId = 'default') {
    ...
    // ✅ NEW: Detect spoiler media tags with ; markers: ;[[media:random:2]];
    const spoilerMediaTagRegex = /;(\[\[media:[^\]]+\]\]);/g;
    const normalMediaTagRegex = /\[\[media:[^\]]+\]\]/g;

    const allMatches = [];
    let match;

    // Find all spoiler media tags first
    while ((match = spoilerMediaTagRegex.exec(postText)) !== null) {
      allMatches.push({
        fullMatch: match[0],          // ;[[media:random:2]];
        tag: match[1],                 // [[media:random:2]]
        shouldMarkAsSpoiler: true,
        index: match.index
      });
    }

    // Find all normal media tags (not wrapped in ;)
    const tempText = postText.replace(spoilerMediaTagRegex, ''); // Remove spoiler tags temporarily
    while ((match = normalMediaTagRegex.exec(tempText)) !== null) {
      allMatches.push({
        fullMatch: match[0],          // [[media:random:2]]
        tag: match[0],                 // [[media:random:2]]
        shouldMarkAsSpoiler: false,
        index: match.index
      });
    }

    console.log(`[RESOLVE_MEDIA] Found ${allMatches.length} media tags (${allMatches.filter(m => m.shouldMarkAsSpoiler).length} with spoiler mark)`);
    ...
    if (allMatches.length > 0) {
      ...
      for (const tagMatch of allMatches) {
        console.log(`[RESOLVE_MEDIA] Resolving tag: ${tagMatch.tag} (spoiler: ${tagMatch.shouldMarkAsSpoiler})`);
        const media = await this.media.resolveMediaTag(tagMatch.tag, profileId);
        console.log(`[RESOLVE_MEDIA] Resolved ${media.length} media file(s)`);

        // ✅ Mark each resolved media with spoiler flag from tag OR random chance
        const settings = await this.storage.getSettings();
        const spoilerChance = settings.mediaSpoilerChance || 0;

        const mediaWithSpoilerFlag = media.map(m => {
          // If tag has explicit spoiler mark (;[[media:...]];), use it
          if (tagMatch.shouldMarkAsSpoiler) {
            return { ...m, shouldMarkAsSpoiler: true };
          }

          // Otherwise, apply random chance
          const randomSpoiler = spoilerChance > 0 && (Math.random() * 100) < spoilerChance;
          if (randomSpoiler) {
            console.log(`[RESOLVE_MEDIA] Random spoiler triggered for media (${spoilerChance}% chance)`);
          }
          return { ...m, shouldMarkAsSpoiler: randomSpoiler };
        });

        resolvedMedia = resolvedMedia.concat(mediaWithSpoilerFlag);
      }

      // Remove all tags from text (both spoiler and normal)
      finalText = postText
        .replace(spoilerMediaTagRegex, '') // Remove ;[[media:...]];
        .replace(normalMediaTagRegex, '')  // Remove [[media:...]]
        .trim();
    }

    // ✅ FIX: If no media tags but random media attachment is enabled, attach random media
    if (resolvedMedia.length === 0) {
      const settings = await this.storage.getSettings();

      if (this.media.shouldAttachRandom(settings)) {
        console.log(`[RESOLVE_MEDIA] Random media attachment triggered (chance: ${settings.mediaAttachChance}%)`);

        // Get all media from profile
        await this.media.init();
        const tx = this.media.db.transaction(['media'], 'readonly');
        const store = tx.objectStore('media');
        const index = store.index('profile');
        const allMedia = await new Promise((resolve) => {
          const request = index.getAll(profileId);
        ...
        if (allMedia && allMedia.length > 0) {
          // Pick 1 random media file
          const randomIndex = Math.floor(Math.random() * allMedia.length);
          const randomMedia = allMedia[randomIndex];

          // ✅ Apply random spoiler chance to random media attachment
          const spoilerChance = settings.mediaSpoilerChance || 0;
          const shouldMarkAsSpoiler = spoilerChance > 0 && (Math.random() * 100) < spoilerChance;

          if (shouldMarkAsSpoiler) {
            console.log(`[RESOLVE_MEDIA] Random spoiler triggered for random media attachment (${spoilerChance}% chance)`);
          }
          ...
          console.log(`[RESOLVE_MEDIA] Attached 1 random media file: ${randomMedia.name} (spoiler: ${shouldMarkAsSpoiler})`);
        } else {
          console.log(`[RESOLVE_MEDIA] No media available in profile "${profileId}" for random attachment`);
        }
      }
    }
