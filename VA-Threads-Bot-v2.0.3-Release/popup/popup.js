...
async function init() {
  ...
  try {
    ...
    // Load contact information from server (Telegram link, etc.)
    updateContactInfo();
  } catch (error) {
...
Object.entries(dom.tabs).forEach(([key, tab]) => {
  ...
  dom.btnResetMedia.addEventListener('click', async () => {
    if (confirm('Delete corrupted media database and restart fresh?\n\nThis will delete all media profiles and images.')) {
      try {
        ...
        if (window.media && window.media.db) {
          window.media.db.close();
        }
        ...
        await new Promise((resolve, reject) => {
          ...
          request.onblocked = () => {
            ...
            setTimeout(() => {
              indexedDB.deleteDatabase('ThreadsBotMedia').onsuccess = () => resolve();
            }, 500);
          };
        });

        // ✅ FIX: Reinitialize media manager properly
        try {
          window.media = new Media();
          await window.media.init();
          window.media.initialized = true;
          console.log('✅ Media manager reinitialized after reset');

          toast('Media database reset successfully! Fresh start with default profile.', 'success');
        } catch (initError) {
          console.error('Failed to reinitialize media after reset:', initError);
          // Fallback to basic object
          window.media = {
            profiles: { default: 'Default' },
            initialized: false
          };
          toast('Media database deleted, but reinitialization failed. Please reload the popup.', 'warn');
        }
  ...
  // 🆕 MEDIA SPOILER CHANCE SLIDER
  const mediaSpoilerChance = document.getElementById('mediaSpoilerChance');
  const mediaSpoilerChanceValue = document.getElementById('mediaSpoilerChanceValue');

  if (mediaSpoilerChance && mediaSpoilerChanceValue) {
    mediaSpoilerChance.addEventListener('input', async (e) => {
      // ✅ CRITICAL FIX: Update UI state IMMEDIATELY to prevent visual glitch
      const value = parseInt(e.target.value);
      mediaSpoilerChanceValue.textContent = value + '%';
      ui.settings.mediaSpoilerChance = value;

      // Save to storage in background (no debounce for sliders!)
      try {
        await send(MSG.UPDATE_SETTINGS, { patch: { mediaSpoilerChance: value } });
      } catch (error) {
        console.error('Failed to update mediaSpoilerChance:', error);
      }
    });
  }
...
function renderAutoPost() {
  ...
  // 🆕 RENDER MEDIA SPOILER CHANCE SLIDER
  const mediaSpoilerChance = document.getElementById('mediaSpoilerChance');
  const mediaSpoilerChanceValue = document.getElementById('mediaSpoilerChanceValue');

  if (mediaSpoilerChance) {
    const spoilerChanceValue = ui.settings.mediaSpoilerChance || 0;

    // Only update slider value if user is not actively dragging it
    if (document.activeElement !== mediaSpoilerChance) {
      mediaSpoilerChance.value = spoilerChanceValue;
    }

    // Always update display value
    if (mediaSpoilerChanceValue) {
      mediaSpoilerChanceValue.textContent = spoilerChanceValue + '%';
    }
  }
...
// Update contact info (Telegram link) from server
async function updateContactInfo() {
  try {
    const telegramLink = document.getElementById('telegramLink');
    if (!telegramLink) return;

    // Fetch contact info from public endpoint (no auth required)
    const response = await fetch('https://us-central1-va-threads-bot.cloudfunctions.net/getContactInfo');

    if (!response.ok) {
      console.warn('Failed to fetch contact info, using defaults');
      return;
    }
    ...
    if (data.success && data.data) {
      const { telegramUsername, telegramUrl } = data.data;

      // Update link href and text
      telegramLink.href = telegramUrl || 'https://t.me/threadsva';
      telegramLink.textContent = `@${telegramUsername || 'threadsva'}`;
    }
  } catch (error) {
    console.error('Failed to update contact info:', error);
    // Silently fail - defaults are already in HTML
  }
}
