...
class AutoPostEngine {
    ...
    try {
      ...
      try {
        ...
        if (error.isLicenseError) {
          await this.log.error('LICENSE_ERROR', `License error: ${error.message}`, { module: 'AUTOPOST' });
          // Stop bot immediately - don't schedule retry
        ...
        // It's a network error - safe to retry
        await this.log.error('API_ERROR', `Failed to get action from server: ${error.message}`, { module: 'AUTOPOST' });

        // ✅ IMPORTANT: Update nextPostAt to prevent "Updating..." freeze
        const FAST_RETRY_DELAY = 60 * 1000; // 1 minute
        const retryAt = Date.now() + FAST_RETRY_DELAY;

        await this.storage.mutateRuntime((runtime) => {
          runtime.nextPostAt = retryAt; // Fix UI freeze
          runtime.apiRetrying = true;
          return runtime;
        });

        await this.alarms.schedule(ALARM.AUTOPOST_TICK, retryAt);
        await this.log.info('AUTOPOST_RETRY', `Retrying in ${FAST_RETRY_DELAY/1000}s...`, { module: 'AUTOPOST' });
        return;
      }
  ...
  parsePosts(raw) {
    ...
    // Normalize ALL line break formats (Word, Excel, Google Docs, etc.)
    const normalized = raw
      .replace(/^\uFEFF/, '')        // Remove UTF-8 BOM
      .replace(/\r\n/g, '\n')        // Windows CRLF -> LF
      .replace(/\r/g, '\n')          // Mac CR -> LF
      .replace(/\u2028/g, '\n')      // Unicode Line Separator -> LF
      .replace(/\u2029/g, '\n\n');   // Unicode Paragraph Separator -> double LF
...
class ActivityEngine {
  ...
  async onStart() {
    ...
    // ✅ ULTRA-ROBUST FIX: Compare calendar dates (YYYY-MM-DD) instead of timestamps
    // This avoids DST issues, type mismatches, and millisecond drift
    const getTodayDateString = (timestamp, timezone) => {
    ...
    const todayDateString = getTodayDateString(Date.now(), settings.timezone);
    const planDateString = runtime.sessionPlanDate
      ? getTodayDateString(runtime.sessionPlanDate, settings.timezone)
      : null;

    // ✅ DEBUG: Log both dates for troubleshooting
    console.log(`[PLAN_CHECK] Today: ${todayDateString}, Plan: ${planDateString}, PlanTimestamp: ${runtime.sessionPlanDate}`);

    // ✅ NEW LOGIC: Compare date strings instead of timestamps
    const needsNewPlan =
      !runtime.sessionPlanToday ||
      runtime.sessionPlanToday.length === 0 ||
      !runtime.sessionPlanDate ||
      !planDateString ||
      planDateString !== todayDateString;
    ...
    if (needsNewPlan) {
      ...
      } else if (!planDateString) {
        reason = 'Plan date is invalid/corrupted';
      } else if (planDateString !== todayDateString) {
        reason = `Plan is from ${planDateString} (stale), today is ${todayDateString}`;
      }
      ...
      const isStale = planDateString && planDateString !== todayDateString;
  ...
  async createDailyPlanFromServer() {
    try {
      ...
      try {
        ...
        // ✅ If license error, stop bot immediately
        if (fetchError.isLicenseError) {
          await this.log.error('LICENSE_ERROR', `License error: ${fetchError.message}`, { module: 'ACTIVITY' });
          await chrome.runtime.sendMessage({
            type: 'LICENSE_EXPIRED',
            message: fetchError.message
          }).catch(() => {});
          throw fetchError; // Propagate to stop activity
        }
      ...
      // ✅ If license error, propagate to stop bot
      if (error.isLicenseError) {
        throw error;
      }
  ...
  async fetchBehaviorProfile() {
    try {
      ...
      // ✅ If license error, stop bot
      if (error.isLicenseError) {
        await this.log.error('LICENSE_ERROR', `License error: ${error.message}`, { module: 'ACTIVITY' });
        await chrome.runtime.sendMessage({
          type: 'LICENSE_EXPIRED',
          message: error.message
        }).catch(() => {});
        throw error; // Stop activity
      }

      await this.log.error('BEHAVIOR_PROFILE_FAIL', error.message, { module: 'ACTIVITY' });
      // Fallback to default behavior if server fails (network errors only)
      this.behaviorConfig = null;
    }
  }
